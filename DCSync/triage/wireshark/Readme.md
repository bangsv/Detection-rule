...
Файл был получен используя утилиту mimikatz под УЗ system. При подобной технике, не используется интерфейс DRSUAPI и при успешной атаке в журналах нет события 4662. (Событие 4662 формируется после вызова функции DsGetNCChanges (Функция из DRSUAPI)  из-за выставленного SACL. Посмотреть и выставить SACL можно в оснастке там же, где и предоставляются права доступа. Также дополнительно нужно перейти на вкладку Advanced.)

SACL — системный список управления доступом в операционной системе Windows. Используется для аудита доступа к объекту.  SACL содержится в дескрипторе безопасности для защищаемого объекта. Каждая запись в списке — запись управления доступом (ACE). ACE определяет типы попыток доступа со стороны указанного доверенного лица, которые заставляют систему создавать запись в журнале событий безопасности. 
